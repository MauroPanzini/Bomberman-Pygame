...
class Enemigo(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        for enemigo in juego.enemigo_sprite:
            if enemigo.id == 1 or enemigo.id == 3:
                enemigo.rect.x += enemigo.velocidad
                if enemigo.velocidad < 0:
                    enemigo.imagen_actual = (self.imagen_actual + 1) % len(self.imagenes_izquierda)
                    enemigo.image = self.imagenes_izquierda[self.imagen_actual]
                else:
                    enemigo.imagen_actual = (self.imagen_actual + 1) % len(self.imagenes_derecha)
                    enemigo.image = self.imagenes_derecha[self.imagen_actual]
            else:
                enemigo.rect.y += enemigo.velocidad
                if enemigo.velocidad < 0:
                    enemigo.imagen_actual = (self.imagen_actual + 1) % len(self.imagenes_arriba)
                    enemigo.image = self.imagenes_arriba[self.imagen_actual]
                else:
                    enemigo.imagen_actual = (self.imagen_actual + 1) % len(self.imagenes_abajo)
                    enemigo.image = self.imagenes_abajo[self.imagen_actual]
            for bloque in juego.bloques:
                if enemigo.hitbox.colliderect(bloque.hitbox):
                    enemigo.velocidad *= -1

            for caja in juego.cajas:
                if enemigo.hitbox.colliderect(caja.hitbox):
                    enemigo.velocidad *= -1

            for bomba in juego.bombas:
                if enemigo.hitbox.colliderect(bomba.hitbox):
                    enemigo.velocidad *= -1

        for enemigo in juego.enemigo_sprite:
            for explosion in juego.explosiones:
                if enemigo.hitbox.colliderect(explosion.hitbox):

                    juego.enemigo_sprite.remove(enemigo)
                    juego.jugador.puntaje += 100
